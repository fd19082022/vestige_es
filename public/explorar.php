...
<main class="contenedor principal">
  ...
  <form method="get" class="filtros" style="display:grid; gap:.6rem; grid-template-columns: repeat(auto-fit, minmax(220px,1fr)); margin-bottom:1rem;">
    ...
    <select name="cat">
      <option value="0">Todas las categorí­as</option>
    ...
    <input type="number" step="0.01" name="min" value="<?= $min !== null ? htmlspecialchars($min) : '' ?>" placeholder="Precio mÃ­n. Bs">
    <input type="number" step="0.01" name="max" value="<?= $max !== null ? htmlspecialchars($max) : '' ?>" placeholder="Precio mÃ¡x. Bs">
          ...
          <div class="card__body" style="padding:.8rem;">
            ...
            <p class="card__categoria" style="margin:.2rem 0; font-size:.9rem; opacity:.8;">
              ...
              <?php if (!empty($p['condicion'])): ?>
                Â· <?= htmlspecialchars($p['condicion']) ?>
              <?php endif; ?>
              <?= !empty($p['es_subasta']) ? ' Â· Subasta' : '' ?>
            </p>
          </div>
        </a>
      </article>
